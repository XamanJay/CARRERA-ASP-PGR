 ...
 <!--- ---JVI-------------------------------------------------------------------------->
    ...
    <% Ob_C2.CLOSE%>
     ...
     <TR>
        ...
        <TD height="30" width="151"> 
          <SELECT NAME='Id_CARcUSR' SIZE=1 onchange='Refresca_Combo3()'>
	   
	         <%DO UNTIL Ob_C3.Eof%>
                <% IF Ob_C3("Id_CARcUSR")= CLNG(VCombo3) THEN %>
                   <OPTION SELECTED VALUE='<%=Ob_C3("Id_CARcUSR")%>'> <%= Ob_C3("CARcUSR")%>
                   </OPTION>
             <% ELSE  %>
                <OPTION VALUE='<%=Ob_C3("Id_CARcUSR")%>'> <%= Ob_C3("CARcUSR")%>
                </OPTION>
             <%END IF  %>
	         <%Ob_C3.MOVENEXT %>
	         <%LOOP%>
