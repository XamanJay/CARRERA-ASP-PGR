...
SUB SReporteAltasCambios
        ...
        SQL_PREV = "select AA.CARcSubprocuradurias,AA.CARcDG_o_Delegaciones, AA.Operacion, count(*)as total from ( "
        SQL_POST = ") AA group by  AA.CARcSubprocuradurias,AA.CARcDG_o_Delegaciones, AA.Operacion "&_
                   " order by  AA.CARcSubprocuradurias,AA.CARcDG_o_Delegaciones, AA.Operacion  "

        Sql =" SELECT   distinct C.CARcSubprocuradurias , D.CARcDG_o_Delegaciones, A.RFC, A.Operacion " &_
					  " FROM         dbo.CARdRastreo_bit A INNER JOIN " &_
                      " dbo.CARdDatosPersonales B ON A.RFC = B.RFC " 

         Sql2 =       " INNER JOIN dbo.CARcSubprocuradurias C ON c.Id_CARcSubprocuradurias = B.Id_CARcSubprocuradurias INNER JOIN "&_
                      " dbo.CARcDG_o_Delegaciones D ON D.Id_CARcDG_o_Delegaciones = B.Id_CARcDG_o_Delegaciones" &_
                      " WHERE     (A.RFC IS NOT NULL) AND (A.RFC <> '') AND  A.IDLOGIN IS NOT NULL AND "&_
                      " ((A.Operacion = 'Alta' AND A.TablaUtilizada = 'CARdDatosPersonales') " &_
                      " OR A.Operacion = 'Cambio') " &_
                      " GROUP BY C.CARcSubprocuradurias, D.CARcDG_o_Delegaciones,A.RFC,  A.Operacion " 
...
// -->
 ...
 <tr> 
	...
	<td colspan="5"><p align="center">
		 <h4><center>PROCURADURÍA GENERAL DE LA REPÚBLICA </center>
		 </h4>
