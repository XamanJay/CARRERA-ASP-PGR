...
'     RootFolderName = "C:\AppsProd\PGR\CarreraPGR\Desarrollo\Datos\Imagenes\"   (Cuando estaba en el 167)
RootFolderName = "C:\AppsProd\COPLADII\CARRERAPGR\Desarrollo\Datos\IMAGENES\"
	
	'\\10.3.194.167\d\AppsProd\PGR\CarreraPGR\Desarrollo\Datos\IMAGENES\IMAGENES
...
'''   IF Nombre_Archivo_Original = "" THEN   ''''' Modificación el dia 4/09/2006
...
'			Matriz2(x) = objUpload.Form(Ob_RS(x).NAME)
              IF INSTR(UCASE(objUpload.Form(Ob_RS(x).NAME)), CHR(39)) <> 0 THEN 
				 'Matriz2(x) = REPLACE(Matriz2(x),CHR(39), CHR(39)+CHR(39))
				 Matriz2(x) = REPLACE(Matriz2(x),"'", "''")
				  Matriz2(x) = REPLACE(Matriz2(x),"&", "")
	'		     RESPONSE.WRITE Matriz2(x)
	'			 RESPONSE.END
			  END IF 
			campos1 = campos1+Ob_RS(X).NAME+","
			campos2 = campos2+"'"+Matriz2(x)+"',"
	      end if  '---(UCASE(Ob_RS(X).NAME) <> "FOTO")
	   end if	'---UCASE(MID(Ob_RS(x).NAME,1,5))= "FECHA"	
	ELSE
...
'			campos2 = campos2&supprocu&", "
		...
		ELSE
		   ...
		   END IF
		END IF 
END IF '--(mid(Ob_RS(X).NAME,1,3) <> "Id_") 
