...
<link href="bubbleicious.css" rel="stylesheet" type="text/css">
...
Path = "https://"+ session("IP_SERVER") +"/CarreraDocs/" + Nombre_Archivo

FOR x = 0 TO total_campos-1
	REDIM Matriz2 (x)


    IF (Ob_RS(x).NAME) <> "Fecha_registro_sistema" and (Ob_RS(x).NAME) <> "usrname" and (mid(Ob_RS(X).NAME,1,3) <> "Id_") THEN
	   if (UCASE(Ob_RS(X).NAME) <> "LK_ARCHIVO") AND (UCASE(Ob_RS(X).NAME) <> "FECHA_GRAB_ARCH") AND (UCASE(Ob_RS(X).NAME) <> "HORA_GRAB_ARCH")THEN
	...
	ELSE
		...
		Matriz2(x) = objUpload.Form(var_combo)

		IF (Ob_RS(x).NAME) <> "Fecha_registro_sistema" and (Ob_RS(x).NAME) <> "usrname" and UCASE(Ob_RS(x).NAME)<>"ID_"+ var_pstabla THEN
			campos1 = campos1+Ob_RS(x).NAME+","
			campos2 = campos2+"'"+Matriz2(x)+"',"
...
var_fecha = Date
IF UCASE(Ob_RS(X).NAME) = "FECHA_GRAB_ARCH" THEN
			campos1 = campos1+Ob_RS(x).NAME+","
			campos2 = campos2&"'"&var_fecha&"',"
END IF	


var_hora = HOUR(TIME)&":"&MINUTE(TIME)&":"&SECOND(TIME)
IF UCASE(Ob_RS(X).NAME) = "HORA_GRAB_ARCH" THEN
			campos1 = campos1+Ob_RS(x).NAME+","
			campos2 = campos2&"'"&var_hora&"',"
END IF	



IF UCASE(Ob_RS(X).NAME) = "LK_ARCHIVO" THEN
			campos1 = campos1+Ob_RS(x).NAME+","
...
<%	
	...
	for each item in objUpload.Files
		...
		Response.Write "<TD>" & var_fecha & "</TD></TR>"
