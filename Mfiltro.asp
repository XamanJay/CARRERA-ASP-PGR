...
''''VG46PROC''ojo con laas mayuscyulas y minusculas
	ELSEIF USUARIO_1 = "VG46PROC" AND Tabla = "CARdDatosPersonales" THEN			
	
			Sql = Sql 
	ELSEIF USUARIO_1 = "VG46PROC" AND Tabla = "CARdFicha" THEN			
	
			Sql = Sql 
	ELSEIF USUARIO_1 = "VG46PROC" AND Tabla = "CARdEscolaridad" THEN			
	
			Sql = Sql 
	ELSEIF USUARIO_1 = "VG46PROC" AND Tabla = "CARdFamilia" THEN			
	
			Sql = Sql 
	ELSEIF USUARIO_1 = "VG46PROC" AND Tabla = "CARdExperienciaLaboral" THEN			
	
			Sql = Sql 
	ELSEIF USUARIO_1 = "VG46PROC" AND Tabla = "CARdIdiomas" THEN			
	
			Sql = Sql 
	ELSEIF USUARIO_1 = "VG46PROC" AND Tabla = "CARdDomicilios" THEN			
	
			Sql = Sql 
   
''''VG46PROC					
					
	ELSEIF USUARIO_1 = "COPLADII36ICAP" AND Tabla = "CARdDomicilios" THEN			
	
			Sql = Sql '& " AND CARdDatosPersonales.CARRERA = 1"

	ELSEIF USUARIO_1 = "COPLADII36CEDH" AND Tabla = "CARdDomicilios" THEN			
    ...
    IF NOT Ob_RS_Id.EOF THEN
     IF INSTR( UCASE(Ob_RS_Id(0)),"PERSONA" ) = 0  THEN
      ...
      IF LEFT(OB_RS2(X).NAME,2) <> "D_"   THEN   ' Este Campo ES de Designaciones Especiales y no se puede ver en CarreraPGR
         ...
         END IF
         IF ETIQ <> "" THEN
            IF uCASE(ETIQ) = "¿VALIDADO?" THEN
               IF UCASE(SESSION("PERFIL")) = "PERFIL 1" OR UCASE(SESSION("PERFIL")) = "PERFIL 2" OR UCASE(SESSION("PERFIL")) = "PERFIL 3"  THEN %>
...
<% NEXT %>
  ...
  <%  IF Tabla = "CARDDATOSPERSONALES" Or Tabla = "CARdDatosPersonales" THEN %>
  <Td class="Encabezados"><B><font color="#FFFFFF">Acción </font></B></Td>
  <% END IF %>
  </TR>
  ...
  <TR  class="Encabezados" > 
    ...
    <% END IF %>
     ...
     <% ELSE %>
        ...
        <%   IF  uCASE(ob_RS2(Campo).NAME) = "CARCVALIDADO" THEN  
                IF (UCASE(SESSION("PERFIL"))= "PERFIL 1" OR UCASE(SESSION("PERFIL")) = "PERFIL 2" OR UCASE(SESSION("PERFIL")) = "PERFIL 3") THEN%>
                    <TD><%= Ob_RS2(Campo) %> </TD>
...
' en la siguiente linea pone el campo OBSERVACIONES  al final de la tabla%>
		...
		<TD>  <a href="EliminarUnRegistrodePersonalSustantivo.asp?Id_CardDatosPersonales=<%=idPer%>"><font size="2" color="#FFFFFF">Eliminar</font></a> </TD>
		 <%   END IF %>
