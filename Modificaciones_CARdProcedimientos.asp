 ...
 <!--- insertar resulstado de  LOOP ----------------------------------------------------------------------------------------------- --->
    ...
    <% 
     ...
     if vModi = "S" then  
        ...
        Ob_RS("CARdProcedimientos")=UCASE(request("CARdProcedimientos")) 
          Ob_RS("Id_CARcTipoProcedimiento")=request("Id_CARcTipoProcedimiento") 
          Ob_RS("Imputacion")=UCASE(request("Imputacion")) 
          Ob_RS("NumeroProcedimiento")=UCASE(request("NumeroProcedimiento")) 
        Ob_RS("AutoridadInstuctora")=UCASE(request("AutoridadInstuctora"))
'        if request("FechaProcedimiento")  <> "" then  Ob_RS("FechaProcedimiento")=request("FechaProcedimiento") end if
         Ob_RS("Id_CARcEstatusProcedimiento")=request("Id_CARcEstatusProcedimiento")
        Ob_RS("Resolucion")=UCASE(request("Resolucion")) 

 IF request ("FechaProcedimiento") = "" then 
    Ob_RS("FechaProcedimiento")= NULL 
 ELSE
    Ob_RS("FechaProcedimiento")= request("FechaProcedimiento") 
 END IF 

 IF request ("FechaResolucion") = "" then 
    Ob_RS("FechaResolucion")= NULL 
 ELSE
    Ob_RS("FechaResolucion")= request("FechaResolucion") 
 END IF 
...
-->	   <TEXTAREA NAME='Imputacion' style="text-transform: uppercase" SIZE=10 ROWS=5 COLS=35><%=Imputacion%></TEXTAREA>
      </TD > 
     <TR>
      <TD>
