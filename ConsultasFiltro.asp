...
<%
  ...
  ID = REQUEST.FORM("ID") 
  DIM CONSULTA4
  CONSULTA4 = REQUEST.FORM("CONSULTA4")

  
  IF CONSULTA4 = 4 THEN
      var_pagina_regreso = "'Consultas_CARdDatospersonales_RED.asp'"
	  
	  RFC1               = REQUEST.FORM("RFC")
	  ApellidoPaterno1   = REQUEST.FORM("ApellidoPaterno")
	  ApellidoMaterno1   = REQUEST.FORM("ApellidoMaterno")
...
SQL = SQL + cActual
  IF UCASE(TABLA) = "CARDADSCRIPCIONES" THEN
...
''___________________ Anexo a la cadena   JVI <>}}}}*>
    DIM USUARIO
	USUARIO = Session("UserLogin")

IF USUARIO = "OAI49"  OR USUARIO = "OAI49_C" OR USUARIO = "VG46PROC" OR USUARIO = "INACIPE39" OR USUARIO = "COPLADII36ICAP" OR USUARIO = "COPLADII36CEDH" OR USUARIO = "COPLADII36DGFP" OR USUARIO = "OM40DGRH" THEN
	...
	ELSE 
			...
			IF Session("TipoUSR") = 4 THEN
			 ...
			 Sql = Sql & " ORDER BY CARdDatosPersonales "
			 
			ELSEIF Session("TipoUSR") = 5 THEN
			
			  Sql = Sql & " AND "&Tabla&".Id_CARcSubprocuradurias = " & session("Subprocuraduria") & " ORDER BY CARdDatosPersonales "
			  
			ELSEIF Session("TipoUSR") = 6 THEN
			  Sql = Sql & " AND "&Tabla&".Id_CARcSubprocuradurias = " & session("Subprocuraduria") & " AND "&Tabla&".Id_CARcDG_o_Delegaciones = " & Session("Delegacion")  & " ORDER BY CARdDatosPersonales"
			...
			ELSEIF Session("TipoUSR") = 7 THEN
			  Sql = Sql & " AND "&Tabla&".Id_CARcSubprocuradurias = " & session("Subprocuraduria") & " AND "&Tabla&".Id_CARcDG_o_Delegaciones = " & Session("Delegacion") & _
						  " AND "&Tabla&".Id_CARcDir_o_subsede = " & Session("Subsede") & " ORDER BY CARdDatosPersonales "
			ELSE
...
if  request("gFiltro") = "SI" THEN
    Sql = session("xQuery")
   ' response.write "Despues de paginar>>>>>>>"+Sql
''   response.end
END IF 
...
Ob_RS2.Open Sql, Ob_Conn, 1,1
			...
			IF EXCEL = 100 THEN      ''''IF         DEL       100             *********************
				''Si es igual a 100 no se pagina y lo envío a excel
				
				Num_Campos= Ob_RS2.FIELDS.COUNT
				'Numero = Ob_RS2.RecordCount
'				RESPONSE.WRITE "---->"&Numero
	
'Num_Campos = Num_Campos-Campos_Per 
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
			IF NOT Ob_RS2.EOF THEN '*****-----**** CONECTOR: 100%>
  ...
  FOR x = 2 TO Num_Campos- 9 +xCampos
    ...
    IF NOT Ob_RS_Id.EOF THEN
      ...
      IF  ETIQ <> "" AND instr(UCASE(ETIQ),"PATERNO")=0 AND  instr(UCASE(ETIQ),"MATERNO")=0  then
         ...
         IF NOT((UCASE(SESSION("PERFIL"))<> "PERFIL 1" AND UCASE(SESSION("PERFIL")) <> "PERFIL 2" AND UCASE(SESSION("PERFIL")) <> "PERFIL 3") AND uCASE(ETIQ) = "¿VALIDADO?") THEN 'AND uCASE(Tabla)= "CARDDATOSPERSONALES" THEN
    %>
...
<%       END IF
      ELSE
         IF instr(UCASE(ETIQ),"PATERNO") <>0 THEN
    %>
              <TD class="Encabezados">
              <P ALIGN="center">
              <B>		<font color="#FFFFFF"> 	Datos de la Persona  </font>	</B> 
<%         END IF
      END IF
   else  
...
<%  IF UCASE(TABLA) = "CARDADSCRIPCIONES" THEN %>
  <Td class="Encabezados"><B><font color="#FFFFFF">Nombre</font></B></Td>
  <Td class="Encabezados"><B><font color="#FFFFFF">UR_Subprocuraduría</font></B></Td>
  <Td class="Encabezados"><B><font color="#FFFFFF">UR_Delegación</font></B></Td>
  <Td class="Encabezados"><B><font color="#FFFFFF">UR_Dirección / Subsede</font></B></Td>
<%  END IF %>
...
<% IF UCASE(TABLA) = "CARDADSCRIPCIONES" THEN %>
             <%  if TRIM(UCASE(Ob_RS2("CARcSubprocuradurias"))) = "SELECCIONAR"  THEN %>
		   	           <TD>&nbsp;</TD>
             <% ELSE %>			 
		   	         <TD><%= Ob_RS2("CARcSubprocuradurias") %> </TD>
             <% END IF %> 
             <%  if TRIM(UCASE(Ob_RS2("CARcDG_o_Delegaciones"))) = "SELECCIONAR"  THEN %>
		   	           <TD>&nbsp;</TD>
             <% ELSE %>			 
		   	         <TD><%= Ob_RS2("CARcDG_o_Delegaciones") %> </TD>
             <% END IF %> 
             <%  if TRIM(UCASE(Ob_RS2("CARcDir_o_subsede"))) = "SELECCIONAR"  THEN %>
		   	           <TD>&nbsp;</TD>
             <% ELSE %>			 
		   	         <TD><%= Ob_RS2("CARcDir_o_subsede") %> </TD>
             <% END IF %> 
<%  END IF %>
 <% IF Tabla = "CARDDATOSPERSONALES" THEN %>
		<TD> <%= Ob_RS2(1) %> &nbsp;</TD>
...
END IF    '*****-----**** CONECTOR: Ob_RS2.EOF
			...
			ELSE   '''***************************** """""""""   DEL 100   """""""""""""
			 EXCEL = 0 
'=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-BITACORA	
		 SET BIT_NUM_REG = SERVER.CREATEOBJECT("ADODB.RECORDSET") 
		 
        Numero_Reg = Ob_RS2.RecordCount
	'''	RESPONSE.WRITE "---->"&Numero_Reg
 ...
 CONSULTA4 = REQUEST.FORM("CONSULTA4")
  ...
  IF CONSULTA4 = 4 THEN
			  ...
			  RFC1               = UCASE(REQUEST.FORM("RFC"))
			  ApellidoPaterno1   = UCASE(REQUEST.FORM("ApellidoPaterno"))
			  ApellidoMaterno1   = UCASE(REQUEST.FORM("ApellidoMaterno"))
			  Nombre1            = UCASE(REQUEST.FORM("Nombre"))
			...
			TABLA_CONSULTADA    = "DatosPersonales"
		   
		   
				
		SQL_BIT_NUM_REG = "INSERT CARdRastreo_bit (CARdRastreo_bit, IdLogin,      IP_user, Fecha,          Operacion,    TablaUtilizada) " & _
						  "VALUES                 ('--RFC : "&RFC1&" --A.Paterno : " &ApellidoPaterno1  &"--A.Materno : "&ApellidoMaterno1&"-- Nombre : "&Nombre1&" -- No de Registros Desplegados : "&Numero_Reg&"',"&Id_Login &", '"&IP&"', '"&FECHA_BIEN&"', '"&OPERACION&"', '"&TABLA_CONSULTADA&"')"      
		
				BIT_NUM_REG.Open SQL_BIT_NUM_REG, Ob_Conn, 1,1
  ELSE
		 var_pagina_regreso = 1
  END IF
...
<BR>
  ...
  <Td class="Encabezados"><B><font color="#FFFFFF"> </font></B></Td>
<% END IF %>
  <!--Td class="Encabezados"><B><font color="#FFFFFF">Datos de la Persona </font></B></Td-->
...
'		response.write Ob_RS2(Campo).Name 
		%>			
		   <% IF (Ob_RS2(Campo).Name = "CVDAccion") THEN %>
			     <% IF Tabla = "CARDDATOSPERSONALES" THEN %>
					<TD><a href="Carr_DosMarcos03.asp?Id_CardDatosPersonales=<%=Ob_RS2("CVDAccion")%>&TITU=<%= REQUEST("TITU")%>&tabla=<%= tabla%>"><font size="2" color="#0033FF">DETALLE</font></a>
					
<!--					        <a href="EliminarUnRegistrodePersonalSustantivo.asp?Id_CardDatosPersonales=<%=Ob_RS2("CVDAccion")%>"><font size="2" color="#FFFFFF">Borrar</font></a><br><HR>
-->
			     <TD><%'= Ob_RS2("RFC") %> </TD>
			     <% ELSE %>
					<TD><a href="Carr_DosMarcos03.asp?Id_CardDatosPersonales=<%=Ob_RS2("Id_CARdDatosPersonales")%>&TITU=<%= REQUEST("TITU")%>&tabla=<%= tabla%>"><font size="2">Detalle</font></a><br>
...
END IF
      ...
      <% ELSE  %>
	      <% if left(ob_RS2(Campo).NAME,2)<> "D_" THEN ' Este campo es de DESIGNACIONES ESPECIALES y no se puede mostrar en Carrerapgr      %>
        <%   IF NOT((UCASE(SESSION("PERFIL"))<> "PERFIL 1" AND UCASE(SESSION("PERFIL")) <> "PERFIL 2" AND UCASE(SESSION("PERFIL")) <> "PERFIL 3") AND uCASE(ob_RS2(Campo).NAME) = "CARCVALIDADO") THEN 'AND uCASE(Tabla)= "CARDDATOSPERSONALES" THEN %>
            <%  if TRIM(UCASE(ob_RS2(Campo))) = "SELECCIONAR"  THEN ' Este campo es de DESIGNACIONES ESPECIALES y no se puede mostrar en Carrerapgr      %>
		   	           <TD>&nbsp; </TD>
             <% ELSE %>			 
		   	         <TD><%= Ob_RS2(Campo) %> </TD>
