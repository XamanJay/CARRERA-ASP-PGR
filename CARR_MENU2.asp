...
IF OP = "PER" THEN
   ...
   CALL PantallaValida("CARdEscolaridad", "Historial Académico")
   ...
   CALL PantallaValida("CARdDomicilios", "Domicilios")
...
ELSEIF OP = "TRA" THEN 
   ...
   CALL PantallaValida("CARdEvaluaciones", "Evaluaciones")
   CALL PantallaValida("CARdArmas", "Armas")
   CALL PantallaValida("CARdEvaluaArmaFuego", "Portación")
   CALL PantallaValida("CARdEstimulosReconocimientos", "Estímulos y Rec.")
   CALL PantallaValida("CARdFormacionInicial", "Formación Ini.")

ELSEIF OP = "CAP" THEN 
   CALL PantallaValida("CARdCursosPersona", "Profesionalización/Seguimiento")
END IF 
 ...
 '  response.end
    Ob_RS.OPEN consulta, Ob_Conn,2, 3, 1
     IF NOT OB_RS.EOF THEN
        IF Pantalla =  "CARdAdscripciones" THEN
           xCAD = xCAD + "&nbsp;&nbsp;&nbsp;<a  target='mainFrame4' href='MFiltro_Adscripcion.asp?tabla_original="+Pantalla+"&ODBC=SIIFP&IdMaestro=CARdDatosPersonales&Id_CARdDatosPersonales="+IDPER+"&TIT="+Letrero+"'><Font size = 2>"+Letrero+"</font></a>"
        ELSE
           xCAD = "" 
