...
<CENTER><H2><%= "Consultas de " + (MID((Tabla),5,LEN(TABLA))) %></H2> <a href="/carrerapgr/Paginacion_jvi.asp">Paginacion_jvi</a><CENTER>
...
Ob_RS2.pageSize = num_registros
Ob_RS2.absolutePage = pagina_actual
cantidad_campos = Ob_RS2.Fields.Count

registros_mostrados = 0
While (Not Ob_RS2.eof And registros_mostrados < num_registros)
	...
	<TR bordercolor="#136F9F" bgcolor="#EFEFEF">
		<%FOR Campo = 0 TO Num_Campos - 1	
		    ...
		    IF (Ob_RS2(Campo).Name = "CVDAccion") THEN %>
					...
					<TD><a href="Carr_DosMarcos03.asp?Id_CardDatosPersonales=<%=Ob_RS2("CVDAccion")%>&tabla=<%= tabla%>"><font size="2">Detalle</font></a><br>
...
<%
i=1
Do While i < Ob_RS2.PageCount + 1
%>
   <b><a href="ConsultasFiltroPer_2.asp?pag=<%=i%>&tabla_original=<%=Tabla%>&ODBC=<%=ODBC%>&ID=<%=ID%>"><%=i & "  -  "%></a></b>
<%
	i=i+1
Loop
%> &nbsp;</td>
...
<BR>
&nbsp;
<table border="1" cellspacing="1" width="20%" id="AutoNumber2">
  <tr> 
    <td width="40%"> <div align="center"> 
        <INPUT TYPE="button" VALUE="BUSCAR otro registro" ONCLICK="location.href=<%= var_pagina_regreso%>" >
      </div></td>
    <td width="40%"> <div align="center"> 
        <INPUT TYPE="button" VALUE="AGREGAR Registro" ONCLICK="location.href='Altas_CARdDatosPersonales.asp'" >
        <!--a href="Altas_CARdDatosPersonales.asp?oper=ALTA">Agregar Registro</a></td-->
      </div></tr>
</table>
