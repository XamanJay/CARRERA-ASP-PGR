...
v_Tabla = "CARcAreaConocimiento"
   ...
   consulta = "SELECT * FROM CARcAreaConocimiento WHERE Id_CARcAreaConocimiento ="+CSTR(ParID) 
...
'======= Inicializa variables de ambiente ===========
   CARcAreaConocimiento= OB_RS("CARcAreaConocimiento")
...
<link href="Estilo.css" rel="stylesheet" type="text/css">
 ...
 <BODY BGPROPERTIES="fixed" BACKGROUND="softgri1.jpg" TOPMARGIN="0" LEFTMARGIN="0">
  ...
  <CENTER>
  <FORM METHOD="post" ACTION= "Modificaciones_CARcAreaConocimiento.asp" NAME= "FormEdita">
...
TOper = REQUEST("TOper")
 IF TOper = "VER" then%>
    <H2><FONT COLOR= #990000>Ver Area de Conocimiento  </FONT></h2>
     <BR>
    <INPUT TYPE="button" VALUE="Ver otro registro" ONCLICK="location.href='javascript:window.history.back()'   " >
<% ELSE %>
     <H2><FONT COLOR= #990000>Modificar  Area de Conocimiento  </FONT></h2>
     <p ALIGN="center"> &nbsp;
 ...
 <!--- insertar resulstado de  LOOP ----------------------------------------------------------------------------------------------- --->
    ...
    <% 
     ...
     if vModi = "S" then  
        ...
        if request("CARcAreaConocimiento")  <> "" then  Ob_RS("CARcAreaConocimiento")=request("CARcAreaConocimiento") end if
 ...
 <TABLE  border='1'   ID='AutoNumber1' HEIGHT='0'>
     <TR>
      <TD> 
         ...
         <B>Catálogo de Area de Conocimiento</B>:<BR>
      </TD>
      <TD>
	     <SELECT NAME='Id_CARcAreaConocimiento' SIZE=1 >
          <% IF Id_CARcAreaConocimiento = "" OR Id_CARcAreaConocimiento = "0" THEN %>
           ...
           <% END IF  %>
	         <%DO UNTIL Ob_RS2.Eof%>
                <%IF Ob_RS2("Id_CARcAreaConocimiento")= CINT(Id_CARcAreaConocimiento) THEN %>                   <OPTION SELECTED VALUE='<%=Ob_RS2("Id_CARcAreaConocimiento")%>'> <%= Ob_RS2("CARcAreaConocimiento")%>
                   </OPTION>
             <% ELSE  %>
    ...
    <% Ob_RS2.CLOSE%>
     ...
     <TR>
      <TD>
        <B>Area de Conocimiento:<BR>  </font>	</B>
      </TD>
      <TD>
 	   <P ALIGN='left'>
       <TEXTAREA NAME='CARcAreaConocimiento' SIZE=10 ROWS=5 COLS=35><%=CARcAreaConocimiento%></TEXTAREA>
  	<BR><BR>
      </TD > 
</TABLE>
...
<INPUT TYPE="Hidden" NAME=tabla_original Value = CARcAreaConocimiento >
...
function ValidaCampos()
{
 ...
 // no Tiene Campos Obligatorios 
  if (FormEdita.Id_CARcAreaConocimiento.value.search("[^0-9]")  == 0 ) 
  {
       alert(" Debe seleccionar una opcion en EL CAMPO: Catálogo de Area de Conocimiento.");
       return false;
  }
