...
v_Tabla = "CARcDetalle"
   ...
   consulta = "SELECT * FROM CARcDetalle WHERE Id_CARcDetalle ="+CSTR(ParID) 
...
<link href="Estilo.css" rel="stylesheet" type="text/css">
 ...
 <BODY BGPROPERTIES="fixed" BACKGROUND="softgri1.jpg" TOPMARGIN="0" LEFTMARGIN="0">
  ...
  <CENTER>
  <FORM METHOD="post" ACTION= "Modificaciones_CARcDetalle.asp" NAME= "FormEdita">
...
TOper = REQUEST("TOper")
 IF TOper = "VER" then%>
    <H2><FONT COLOR= #3366CC>Ver   Detalle de Est&iacute;mulo y Reconocimiento  </FONT></h2>
     <BR>
    <INPUT TYPE="button" VALUE="Ver otro registro" ONCLICK="location.href='javascript:window.history.back()'   " >
<% ELSE %>
     <H2><FONT COLOR= #3366CC>Modificar    Detalle de Est&iacute;mulo y Reconocimiento  </FONT></h2>
     <p ALIGN="center"> &nbsp;
 ...
 <!--- insertar resulstado de  LOOP ----------------------------------------------------------------------------------------------- --->
    ...
    <% 
     ...
     if vModi = "S" then  
        ...
        if request("CARcDetalle")  <> "" then  Ob_RS("CARcDetalle")=UCASE(request("CARcDetalle")) end if
 ...
 <TABLE  border='1'   ID='AutoNumber1' HEIGHT='0'>
     ...
     <TR>
      <TD> 
         ...
         <B>Tipo de est&iacute;mulo</B><BR>
      </TD>
      <TD>
	     <SELECT NAME='Id_CARcTipoEstimuloRec' SIZE=1 >
          <% IF Id_CARcTipoEstimuloRec = "" OR Id_CARcTipoEstimuloRec = "0" THEN %>
           ...
           <% END IF  %>
	         <%DO UNTIL Ob_RS2.Eof%>
                <%IF Ob_RS2("Id_CARcTipoEstimuloRec")= CINT(Id_CARcTipoEstimuloRec) THEN %>                   <OPTION SELECTED VALUE='<%=Ob_RS2("Id_CARcTipoEstimuloRec")%>'> <%= Ob_RS2("CARcTipoEstimuloRec")%>
                   </OPTION>
             <% ELSE  %>
...
<INPUT TYPE="Hidden" NAME=tabla_original Value = CARcDetalle >
...
function ValidaCampos()
{
 ...
 // no Tiene Campos Obligatorios 
  if (FormEdita.Id_CARcDetalle.value.search("[^0-9]")  == 0 ) 
  {
       alert(" Debe seleccionar una opcion en EL CAMPO: Catalogo Detalle (Estimulos y Rec).");
       return false;
  }
  
  if (FormEdita.Id_CARcTipoEstimuloRec.value.search("[^0-9]")  == 0 ) 
  {
       alert(" Debe seleccionar una opcion en EL CAMPO: o.");
       return false;
  }
