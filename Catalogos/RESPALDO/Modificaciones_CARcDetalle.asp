...
TOper = REQUEST("TOper")
 IF TOper = "VER" then%>
    <H2><FONT COLOR= #3366CC>Ver de  Detalle de Estimulo y Reconocimiento  </FONT></h2>
     <BR>
    <INPUT TYPE="button" VALUE="Ver otro registro" ONCLICK="location.href='javascript:window.history.back()'   " >
<% ELSE %>
     <H2><FONT COLOR= #3366CC>Modificar  de  Detalle de Estimulo y Reconocimiento  </FONT></h2>
  <p ALIGN="center"> &nbsp;
 ...
 <!--- insertar resulstado de  LOOP ----------------------------------------------------------------------------------------------- --->
    ...
    <% 
     ...
     if vModi = "S" then  
        ...
        if request("CARcDetalle")  <> "" then  Ob_RS("CARcDetalle")=request("CARcDetalle") end if
