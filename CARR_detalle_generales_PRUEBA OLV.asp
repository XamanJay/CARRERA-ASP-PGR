...
'SQL_Ads = "SELECT CARcSubprocuradurias, CARcDG_o_Delegaciones,Adscripcion FROM VISTA_ADSCRIPCIONES_P_DETALLE " & _
SQL_Ads = "SELECT CARcSubprocuradurias, CARcDG_o_Delegaciones FROM VISTA_ADSCRIPCIONES_P_DETALLE " & _
...
'	  RESPONSE.END 
  ...
  <% if Ob_Ads.eof then %>
      <td  align="left" colspan="3"><font color="#48ADBF" size="1" face="Verdana, Arial, Helvetica, sans-serif"> <b><%= "Formación Inicial" %><BR>
            <%= "ICAP") %></b></font></td>
  <% else  %>
       ...
       <td  align="left" colspan="3"><font color="#48ADBF" size="1" face="Verdana, Arial, Helvetica, sans-serif"> <b><%= Ob_Ads("SUBPROCURADURIAS") ><BR><%= Ob_Ads("CARcDG_o_Delegaciones") %></b></font></td>
 <% end if%>
    </tr>
    <tr>
