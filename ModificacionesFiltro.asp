...
If Not Ob_RS2.EoF Then %>
	<center><H2>Registros a modificar</h2><center>
...
<tr>
	<%For Campo = 0 to Num_Campos - 1%>
		   <% IF TRIM(Ob_RS2(Campo).Name) = "CVDAccion" THEN %>
              <TD BGCOLOR = "#00003C">
              <P ALIGN="center">            
              <B>		<font color="#FFFFFF"> 	<%= "Accion" %>  </font>	</B> 
			<%END IF
		NEXT%> 
	...
	<TR>
		<%FOR Campo = 0 TO Num_Campos - 1			
		    IF (Ob_RS2(Campo).Name = "CVDAccion") THEN %>
					<TD><a href="ModificacionesEdicion.asp?lnkIdCarros=<%=Ob_RS2("CVDAccion")%>&parametroTabla=<%= Tabla%>&parametroODBC=<%=ODBC%>&parametroCadena=<%= sql%>&parametroID=<%= ID%>"><font size="2">Modificar</font></a><br>
			<%Else
