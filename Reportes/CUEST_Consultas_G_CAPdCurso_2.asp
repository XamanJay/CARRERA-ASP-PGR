  ...
  <TR   > 
    <% end if %>
      ...
      <% 
					...
					%>
      <input type="Button"   value="VER RESULTADOS" name="btnListar" onClick=" location.href='CUEST_CAP_Resultado_Cuestionarios.asp?vp_id_curso_seleccionado=<%= vl_id_curso_seleccionado%>&vp_curso_seleccionado=<%= vl_curso_seleccionado%>&vp_curso_fecha_inicio=<%= vl_curso_fecha_inicio%>&vp_curso_fecha_fin=<%= vl_curso_fecha_fin%>&vp_curso_sede= <%= vl_curso_sede%>&vp_curso_horario= <%= vl_curso_horario%>&par_instructor_curso= <%= par_instructor_curso%>&par_ins_capacitadora= <%= par_ins_capacitadora%>&par_ins_aula= <%= par_ins_aula%>&tipo_rep= 1'">
...
%>
<a href="https://<%= session("IP_SERVER")%>/cursos/" TARGET="_top"><font size="3">Regresar a consultar convocatorias</font></a>
<%
END IF
%>
