...
<HEAD>  <TITLE>   Pantalla Altas  </TITLE> </HEAD>
 ...
 <BODY TOPMARGIN="0" LEFTMARGIN="0" background="softgri1.jpg">
  ...
  <CENTER>
  <FORM TARGET="BOTONES" METHOD="post" ACTION= "ARMAZONDERESULTADOS_NOMBRAMIENTOS.asp" NAME= "FormAltas">
 ...
 <TABLE class="SL"  border='0'   WIDTH='82%' ID='AutoNumber1' HEIGHT='0'>
     ...
     <TR>
	  <TD width="11%" rowspan="4" class="Encabezados"><em>Listado de personas por nombramiento y estatus laboral. </em></TD>
	...
	<TD class="Encabezados">Estatus laboral: </TD>
    ...
    <TD class="Encabezados" rowspan="2">	  	
	...
	<TR>
	<TD class="Encabezados">Nombramiento: </TD>

	<TD class="DATOS"> 
		   <%Sql ="SELECT * FROM CARcNombramiento ORDER BY  Id_CARcNombramiento DESC" %>
          <% rs2.OPEN Sql, Ob_Conn %>
	                  <SELECT NAME='Id_CARcNombramiento' SIZE=1>
	                <%DO UNTIL rs2.Eof%>
                     <OPTION SELECTED VALUE='<%=rs2("Id_CARcNombramiento")%>'> <%= rs2("CARcNombramiento")%> </OPTION>
	                <%rs2.MOVENEXT %>
	                <%LOOP%>
                      </SELECT>
                     <% rs2.CLOSE%>
